...
namespace WindowsFormsApp1
{
	...
	public partial class laba : Form
    {
        ...
        private void laba_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == (char)Keys.F1)
            { 
			    string[] mini = {};
				string слова = richTextBox1.Text;
				if (!String.IsNullOrWhiteSpace(слова)) 
                {
                    int i = 0;
                    string x = "";
                    foreach (char c in слова)
                    {
                        if (glasn(c)) 
                        { 
                            i++;
					...
					if (mini != null)
                    {
                        if (n < mini.Length)
                        {
                            if (n != 0)
                            {
                                result1 = FindWord(mini[n - 1], richTextBox1.Text.Length);
                                richTextBox1.SelectionStart = result1;
                                richTextBox1.SelectionLength = mini[n - 1].Length;
                                richTextBox1.SelectionBackColor = Color.White;
                            }
                            result1 = FindWord(mini[n], richTextBox1.Text.Length);
                            richTextBox1.SelectionStart = result1;
                            richTextBox1.SelectionLength = mini[n].Length;
                        ...
                        {
							textBox3.Text = ("Больше нечего выделять");
						}
					}
